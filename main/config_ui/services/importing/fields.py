...
class Field(CellOper):
    ...
    def _get_value_or_object(self, ws: Worksheet, row: int, cache: dict[str, dict]):
        ...
        if self.__foreign_key:
            ...
            return self._get_value_for_json_field(value)
        else:
            return value

    def _get_value_for_json_field(self, value):
        try:
            return loads(value)
        except Exception as ex:
            logger.error(f"Ошибка десериализации. Поле: {self.__name}, значение: {value}."
                         f"\nEXCEPTION: {ex}")
            return None
